...
import { isIChatSessionFileChange2, type IChatSessionFileChange } from '../../../../workbench/contrib/chat/common/chatSessionsService.js';
...
// --- Legacy Code Review Types (retained for compatibility with 1.118.1) -------
...
export function getCodeReviewFilesFromSessionChanges(changes: readonly IChatSessionFileChange[]): readonly ICodeReviewFile[] {
...
export function getCodeReviewVersion(files: readonly ICodeReviewFile[]): string {
	const stableFileList = files
		...
		.sort();
	return `v1:${stableFileList.length}:${hash(stableFileList.join(','))}`;
}
