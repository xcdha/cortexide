...
(async function () {
	...
	function setupNLS<T extends ISandboxConfiguration>(configuration: T): void {
		...
		// Check if node_modules.asar exists (only true in production builds)
		try {
			const fs = require('fs');
			const path = require('path');
			const asarPath = path.join(configuration.appRoot, 'node_modules.asar');
			globalThis._VSCODE_NODE_MODULES_ASAR_EXISTS = fs.existsSync(asarPath);
		} catch {
			globalThis._VSCODE_NODE_MODULES_ASAR_EXISTS = false;
		}
